...
def write_workbook(summary, per_emp, out_path, code_gaps=None, recs=None):
    ...
    s.value = (f"{len(summary)} קבצים · הופק "
               ...
               "מנטרל: משרה חלקית ← ותק סטודנט ← ותק קטוע ← בסיס ← גמול "
               "השתלמות ← דריכות בי\"ח ← גמול מנהל. הנותר = שגיאה אמיתית. "
               "% מחושב מתוך 'מלאות פעילות' = תקין + כל דלי השגויים; "
               "ללא-בסיס ורטרו נספרים על כלל העובדים ואינם בשרשרת.")
    ...
    for k in ("part_time", "ft", "ft_valid", "inv_student", "inv_vatek",
              "inv_base", "inv_gmul", "inv_brich", "inv_mnhal", "inv_real"):
    ...
    labels = ["חודש שכר", "קובץ", "עובדים", "משרה חלקית", "משרה מלאה", "תקין (מלאה)",
              "שגויי ותק סטודנט", "שגויי ותק קטוע", "שגויי בסיס", "שגויי גמול",
              "שגויי דריכות", "שגויי גמול מנהל", "שגויים אמיתיים", "ללא בסיס",
              "רטרו רב-תקופתי", "% שגויים אמיתיים"]
    _header_row(ws, head_r, labels,
                [11, 18, 10, 10, 10, 11, 11, 10, 10, 10, 10, 11, 12, 9, 11, 13])
    for i, r in enumerate(summary, start=head_r + 1):
        vals = [r["month"], r["file"], r["workers"], r.get("part_time", 0),
                r.get("ft", 0), r.get("ft_valid", 0), r.get("inv_student", 0),
                r.get("inv_vatek", 0), r.get("inv_base", 0), r.get("inv_gmul", 0),
                r.get("inv_brich", 0), r.get("inv_mnhal", 0), r.get("inv_real", 0),
        ...
        for c_i, v in enumerate(vals, start=1):
            ...
            if c_i in (7, 8, 9, 10, 11, 12) and v:
    ...
    tvals = ["סה\"כ", "", tot["workers"], tot["part_time"], tot["ft"],
             tot["ft_valid"], tot["inv_student"], tot["inv_vatek"],
             tot["inv_base"], tot["inv_gmul"], tot["inv_brich"],
             tot["inv_mnhal"], tot["inv_real"], tot["no_base"],
             tot["multi"], real_pct_tot]
    ...
    ws.conditional_formatting.add(
        f"M{head_r + 1}:M{last}",          # שגויים אמיתיים
        DataBarRule(start_type="num", start_value=0, end_type="max",
                    color="FFD03B3B", showValue=True))

    # ---- פערים לפי סמל שכר (a second table, to the right of the per-file one) ---
    if code_gaps:
        cbase = 18   # column R (leaves a gap after the 16-column per-file table)
