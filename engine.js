...
(function (global) {
  ...
  const NON_PENSIONABLE = new Set([
    // 1622 added 5.8.2026: `מאפייני רכיבי שכר` row 375 declares משכורת קובעת = לא.
    // Clears the largest hole in the 12/2023 coverage gap (26 rows / ₪45,281).
    // 1631 added 5.8.2026: `מאפייני רכיבי שכר` row 380, משכורת קובעת = לא.
    ...
    // 4536 and 5527 added 6.8.2026. 5527: `מאפייני רכיבי שכר` משכורת קובעת = לא,
    // ברוטו = כן. 4536: not declared anywhere. 5527 still drives the ותק סטודנט
    // bucket, which reads the slip component directly.
    // 0, 1027, 1044, 1904 added 6.8.2026. 1027/1044 are declared
    // משכורת קובעת = לא, ברוטו = כן; 1904 is not declared. 0 appears on no
    // slip in any file scanned — see main.py for the caveat about code 0.
    0, 954, 1027, 1044, 1170, 1622, 1623, 1631, 1904, 1927, 4536, 4540, 5274, 5527, 1936, 1934, 1901, 1266, 1260, 903, 889, 4457, 1088, 1711, 4120, 1375, 4133,
    ...
    5272,
    // 4.8.2026, מרשימת הכיסוי של 12/2014: חתימה/עדות-גט, 100% ש.מחלה,
    // נכ.העד-חרום. 1731 מופיע ב-sminimum אך רק בטבלת חוק המינימום —
    // לא כהצהרת פנסיוניות ולא כנוסחה.
    1731, 4123, 4978,
    // שט.יומית א'/ב' — קצובה יומית (4.8.2026, אותה רשימת כיסוי).
    1228, 1229,
